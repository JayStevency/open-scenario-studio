...
엑셀 내보내기(FR-005)와 되돌리기(NFR-04)도 있다. 되돌리기는 이력의 `before` 를 근거로 하며, 되돌린 것 자체도 새 기록으로 남긴다 — 이력은 append-only 라 고치지 않는다.

미착수: 사내 인증(NFR-06), 관리자 목록 설정(FR-107), 시나리오 추가·삭제(FR-206). 되돌리기는 규칙·시나리오의 생성·삭제까지는 다루지 않는다.
